...
def current_party(row):
    """PLPT_NM 등이 '더불어민주당/무소속'처럼 이어진 값일 때 마지막 조각을 현재로 본다.

    주의 — 이 가정은 아직 검증되지 않았다. 2026-08-12 전수 대조에서 62명 중 61명이
    페이지 표기와 일치했고, 유일한 불일치가 장경태 의원(페이지=무소속, API=더불어민주당)
    이었다. 장경태 의원은 2026-03 탈당·2026-04 제명 처분으로 무소속이고 국회가 낸
    소위원회 명단에서도 비교섭단체로 분류돼 있으므로, API 쪽이 갱신되지 않았거나
    이력 정렬이 최신순이어서 이 함수가 가장 오래된 정당을 집고 있거나 둘 중 하나다.
    22대에서 당적을 바꾼 사람이 이 명단에 그 한 명뿐이라 대조군이 없어 판별하지 못했다
    (원본 값 확인을 시도했으나 API 지연으로 실패).

    그래서 일일 감지(cloud_check_updates.py)는 이 함수를 쓰지 않고 원본 문자열을
    통째로 비교한다 — 정렬 방향과 무관하게 안전하다. 이 함수는 사람이 눈으로 보는
    점검용으로만 남겨둔다.
    """
