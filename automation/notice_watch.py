...
# 이 트래커가 찾는 말.
#
# '자전거'를 통째로 두는 데는 두 가지 이유가 있다.
#   - 도로교통법이 "자전거등"이라는 정의어로 자전거와 개인형 이동장치를 묶어 둔다.
#     주차·통행처럼 그 정의어를 쓰는 조항을 고치면 PM에도 그대로 걸린다.
#   - 공유 자전거가 경쟁 영역이라 그쪽 규제도 같이 봐야 한다.
# 부분일치라서 '자전거등'·'공유자전거'·'전기자전거'·'자전거도로'는 이 한 줄로 다 걸린다.
#
# 반대로 '자전거'라는 말이 들어가지 않는 표현은 따로 적어야 한다. 대여사업 규제가
# 그렇다 — PM 대여업과 공유 자전거 사업이 같은 조항에 실리는 일이 많다.
KEYWORDS = [
